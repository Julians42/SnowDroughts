...
ensemble_members = glob("/decp/SPEAR_MED/SPEAR_c192_o1_Hist_AllForc_IC1921_K50/"
                        "pp_ens_*/atmos/ts/monthly/94yr/atmos.192101-201412.t_ref_max.nc")
...
def xr_to_SWEI(ds):
    ...
    ds_swei = ds.copy()
    try:
        ds_swei.t_ref_max.data = xr.DataArray(SWEI_data)
    except:
        ds_swei.t_ref_max.data = xr.DataArray(SWEI_data)
    return ds_swei
...
def save_hist_nc(hist_ens, index):
    ...
    ncfile             = Dataset(f'/work/Julian.Schmitt/data/tmaxi/monthly_ens_{index}.nc',mode='w',format='NETCDF4') 
    # create dimension 
    ...
    for dim in ncfile.dimensions.items():
        print(dim)
    # generate header
    ncfile.title       ='TMAX Index from SPEAR'
    # define variables 
    ...
    time.long_name     = 'time'
    tmax               = ncfile.createVariable('tmax', np.float64,('mask','time')) # note: unlimited dimension is leftmost
    tmax.units         = 'none' 
    tmax.standard_name = 'TMAX_I z-score'  
    print(tmax)
